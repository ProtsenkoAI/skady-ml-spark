...
from model.saving.savers import StandardSaver
...
class TestStandardSaver(unittest.TestCase):
    ...
    def test_simple_model_save_load(self):
        ...
        model_name = saver.save_packaged_model(model, processor)
        source_model_class = type(model)
        source_proc_class = type(processor)
        del model, processor
        loaded_model, loaded_processor = saver.load_packaged_model(model_name)
    ...
    def test_multiple_models_loading(self):
        ...
        model1_name = saver.save_packaged_model(model1, processor1)
        model2_name = saver.save_packaged_model(model2, processor2)
        self.assertNotEqual(model1_name, model2_name, "different models have the same name")

        model1_loaded, proc1_loaded = saver.load_packaged_model(model1_name)
        model2_loaded, proc2_loaded = saver.load_packaged_model(model2_name)
