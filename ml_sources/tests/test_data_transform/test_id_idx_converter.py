...
from recsys_pipeline.data_transform import id_idx_converter
...
class TestIdIdxConverter(unittest.TestCase):
    def test_initializing_with_ids(self):
        ...
        converter = id_idx_converter.IdIdxConverter(*init_ids)

    def test_convert_id_to_idx(self):
        single_id = 123
        converter = id_idx_converter.IdIdxConverter()
    ...
    def test_convert_idx_to_id(self):
        src_elem = 10**5
        converter = id_idx_converter.IdIdxConverter(src_elem)
    ...
    def test_count_unknown(self):
        elems = [1, 2, 3]
        converter = id_idx_converter.IdIdxConverter(*elems)
        new_elems = [2, 3, 4, 5, 6]
    ...
    def test_idx_inverse_transform(self):
        elem_id = 456
        src_idx = 0
        converter = id_idx_converter.IdIdxConverter(elem_id)
