...
from recsys_pipeline.data_transform import id_idx_converter, data_preprocessor
...
class ProdManager:
    ...
    def __init__(self, 
        ...
        # self.trainer = self._create_trainer(model_name)

        self.user_colname = "user_id"
        self.item_colname = "anime_id"
        self.user_id_idx_conv = user_id_idx_conv
        self.item_id_idx_conv = item_id_idx_conv
    ...
    def get_recommends(self, users):
        ...
        all_item_idxs = self.item_id_idx_conv.get_all_idxs()
        print("all_item_ids", all_item_idxs)
        ...
        for user in all_items_preds:
            user_conv = self.item_id_idx_conv.get_ids(*user)
    ...
    def convert_interacts(self, interacts):
        ...
        interacts[self.user_colname] = self.user_id_idx_conv.add_ids_get_idxs(*users)
        interacts[self.item_colname] = self.item_id_idx_conv.add_ids_get_idxs(*items)
    ...
    def update_trainer_with_interacts(self, trainer, interacts):
        users = interacts[self.user_colname]
        items = interacts[self.item_colname]
        number_of_new_users = self.user_id_idx_conv.count_unknown(*users)
        number_of_new_items = self.item_id_idx_conv.count_unknown(*items)

        trainer.add_users(number_of_new_users)
        trainer.add_items(number_of_new_items)

    def save_results(self, new_model_name): #or maybe use self.model_name ?
        raise NotImplementedError

    def _create_trainer(self):
        dataset = self.dataloader_builder(self.interacts)
        if self.model_saver.check_model_exists(self.model_name):
            model = self.model_saver.load(self.model_name)
        else:
