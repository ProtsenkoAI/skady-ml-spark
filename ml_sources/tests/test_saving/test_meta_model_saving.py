...
class TestMetaModelSaver(unittest.TestCase):
    ...
    def test_model_saving_with_custom_path_and_prefix(self):
        ...
        model, loaded_model = self._save_load_standard_model(saver, "model", 10, 10, 10)

        self._assert_parameters_equal(model, loaded_model)
        self.assertTrue(os.path.isfile(self.save_dir + "params_aaa.json"))
        self.assertTrue(os.path.isfile(self.save_dir + "model_omg_weights.pt"))
    ...
    def test_saving_loading_multiple_models(self):
        ...
        saver.save("model1", model1.state_dict(), model1_kwargs)
        saver.save("model2", model2.state_dict(), model2_kwargs)

        loaded_model1 = saver.load("model1")
    ...
    def test_check_model_exists(self):
        ...
        saver.save(model_name, model.state_dict(), model_kwargs)
    ...
    def _save_load_standard_model(self, saver, model_name, nusers, nitems, hidden):
        model_kwargs = {"nusers": 50, "nitems": 50, "hidden_size": 20}
        model = mf_with_bias.MFWithBiasModel(**model_kwargs)
        saver.save(model_name, model.state_dict(), model_kwargs)

        loaded_model = saver.load(model_name)
